...
class WechatApi
{
	...
	const TEMPLATE_GET_TPL_URL = '/template/get_all_private_template?';
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
                ...
                file_put_contents('song.log','将xml赋值:'.$postStr."\r\n",8);
            }
	...
	*/
	...
	public function getTempList()
    {
        ...
        $result = $this->http_get(self::API_URL_PREFIX.self::TEMPLATE_GET_TPL_URL.'access_token='.$this->access_token);
        if($result){
            $json = json_decode($result,true); //将json数据转换成数组
